export default {
  install(Vue: any, options: any) {
    // 验证数据是否为空
    Vue.prototype.$isEmpty = (value: any) => {
      // 判断字符串是否为空
      switch (typeof value) {
        case "undefined":
          return true;
    ...
    // 转换字符
    Vue.prototype.$replaceAllStr = (
      value: string,
      regexp: any,
      substr: string
    ) => {
      if (Vue.$isEmpty(value)) {
        return "";
      }
      return value.replace(new RegExp(regexp, "gm"), substr);
    };
    // 根据value从List中获取对应label值
    Vue.prototype.$getListValue = (
      value: any,
      ...
      let result:
        ...
        | undefined = list.find((el: any) => value === el.value);
    ...
    Vue.prototype.$regSpecial = (str: any) => {
      //
    ...
    // 根据当前时间推算本月总天数
    Vue.prototype.$monthCalcDay = (time: any) => {
      time = new Date(time);
      ...
      let tmpArr: string = [year, month, "1"].join("/");
      let endDayTime = new Date(tmpArr).getTime() - 86400000;
      return new Date(endDayTime).getDate();
    };
    // 比较日期相等
    Vue.prototype.$dateEqual = (time1: any, time2: any) => {
     ...
     */
    Vue.prototype.$tagTime = (time: any, tag: any) => {
      if (!tag) {
        return "";
      }
      if (!time) {
    ...
    Vue.prototype.$debounce = (() => {
      ...
      return (callback: any, wait: any) => {
        ...
        let remaining: any = currentTime - lastTime - wait;
        if (remaining >= 0) {
          callback();
     ...
     */
    Vue.prototype.$displace = (index1: any, index2: any, array: any) => {
      let newArray = [...array],
        tmpVal = newArray[index1],
        max,
        min,
        oneArray,
        twoArray,
    ...
    // 从目标数组中删除给定下标的元素
    Vue.prototype.$deleteIndexOfArray = (index: any, list: any) => {
      if (!Array.isArray(list)) {
        return;
      }
    ...
    // 对传入的参数按指定的标签进行拼接
    Vue.prototype.$splicing = (tag: any, ...arr: any) => {
      arr = arr.filter((el: any) => el);
      return arr.join(tag);
    };
  }
};
